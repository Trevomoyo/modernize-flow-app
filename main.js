...
document.getElementById('contactForm').addEventListener('submit', (e) => {
  ...
  const subject = encodeURIComponent(`New Enquiry from ${name} — ModernizeFlow`);
  const body = encodeURIComponent(
    `Name: ${name}\nEmail: ${email}\nBusiness Type: ${businessType || 'Not specified'}\n\nMessage:\n${message}`
  );

  window.location.href = `mailto:modernizeflow@gmail.com?subject=${subject}&body=${body}`;

  showMsg(msg, '✓ Your email app is opening — just hit send!', 'success');
  form.reset();
});
